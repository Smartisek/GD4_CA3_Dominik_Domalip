//base networking class that handles low level socket operations, packetr queing and bandwidth management. Client and server inherit from this base class

//Server: Listen on port 50000
//Client : Connect to server IP : 50000
//Both : Send / Receive packets via this socket
...
{
public:
	//every packet will start with one of these codes so it can be identified and processed correctly
	static const uint32_t kHelloCC = 'HELO'; //client hello message to server
	...
	bool Init(uint16_t in_port); //binding to a port to listen for incoming packets 
	...
	//functions to monitor bandwidth usage 
	//Every frame :
	...
	//we can simulate bad network conditions with these functions 
...
private:
	...
	private:
		float m_received_time; //when did it arrive
		InputMemoryBitStream m_packet_buffer; // what data did it contain
		SocketAddress m_from_address; //from which address did this come from 
	};
